...
There are differente approaches to feature selection:
...
The Bhattacharyya distance makes use ofthe probability of error of the Bayes classifier, defined as:
...
The integral of a minumum is quite difficult to calculate, so we use an upper bound called **Chernoff bound**:
...
$\sum_{i=1}^n || x_i - \hat x_i ||^2 = \sum_{i=1}^n||x_i - \sum_{k=1}^m{y_{k_{i}}\phi_k}||^2$

These $\phi_k$ are the eigenvectors of the scatter matrix $S$, obtained by multiplying the covariance matrix $\Sigma$ (eigenvectors are the same) by $N$, calculated from the samples $x_i$.
...
In the case of multiclass PCA, we don't have just a vector $w$, but a matrix $W$ with shape $n*(c-1)$, where $c$ is the number of classes, with generalized scatter matrices $S_B$ (with global mean) and $S_W$.
